<?
//example read letters from account
...
*/
$pop3=new pop3_class;
$pop3->hostname="mx.000webhost.com"; /* POP 3 server host name */
$pop3->port=110; /* POP 3 server host port,
usually 110 but some servers use other ports
Gmail uses 995 */
$pop3->tls=0; /* Establish secure connections using TLS */
$user="admin@samsungmobyle.comxa.com"; /* Authentication user name */
$password="newh0p3"; /* Authentication password */
				...
				if(GetType($result)=="array") {
					...
					if($messages>0) {
						for ($msg = 0; $msg < $messages; $msg++) {
							if(($error=$pop3->RetrieveMessage($msg + 1,$headers,$body, 35))=="") {
								echo '<hr><b style="color:red">NEW MESSAGE </b> <br>';
								$in_enc = 'UTF-8';
								$body_is_base_64 = false;
								//echo "<PRE>Message 1:\n---Message headers starts below---</PRE>\n";
								for($line=0;$line<count($headers);$line++) {
									//echo "<PRE>",HtmlSpecialChars($headers[$line]),"</PRE>\n";
									$header_line = HtmlSpecialChars($headers[$line]);
									if (strpos(strtolower($header_line), 'charset=koi8-r') !== false) {
										$in_enc = 'koi8-r';
									}
									if (strpos($header_line, 'Content-Transfer-Encoding: base64') !== false) {
										$body_is_base_64 = true;
									}
									if (strpos($header_line, 'Subject: ') !== false) {
										$subject = trim( str_replace('Subject: ', '', $header_line) );
										if (strpos(strtolower($header_line), '=?koi8-r?b?') !== false) {
											$subject = str_replace('=?koi8-r?b?', '', $subject);
											$subject = str_replace('=?koi8-r?B?', '', $subject);
											$subject = str_replace('?=', '', $subject);
											//echo '<b>Subject: </b> ' . trim($subject) . '<br>';
											//echo '<b>Subject: </b> ' . base64_decode(trim('98/UINMg3NTPyiDUxc3PyiDC1cTVINfPxdfB1Ngh') ) . '<br>';
											//$sss = trim($subject);
											/*if ($sss == '98/UINMg3NTPyiDUxc3PyiDC1cTVINfPxdfB1Ngh') {
												echo '<b style="color:red">TRUE </b> <br>';
											} else {
												echo '<b style="color:red">FALSE </b> <br>';
											}*/
											$subject = trim(base64_decode($subject) );
											$in_enc = 'koi8-r';
											$subject = mb_convert_encoding($subject, 'UTF-8', $in_enc);
										}
										
										if (strpos(strtolower($header_line), '=?utf-8?b?') !== false) {
								...
								//for mails from gmail check is_base_64 again\
								for($line=0;$line<count($body);$line++) {
									$s = $body[$line];
									if (strpos($s, 'Content-Transfer-Encoding: base64') !== false) {
										$body_is_base_64 = true;
									}
								}
								...
								if ($body_is_base_64) {
									...
									for($line=0;$line<count($body);$line++) {
										...
										if (strpos($s, '--') === 0 && preg_replace("#[0-9A-z\-]#", '', $s) === '') {
											continue;
										}
										if (strpos($s, 'Content-Transfer-Encoding:') === 0 || strpos($s, 'Content-Type:') === 0) {
									...
									$s = join("", $a_body);
									//echo '<b style="color:red">body_is_base_64 TRUE </b> <br>';
									$s = base64_decode($s);
									$s = HtmlSpecialChars($s);
									if ($in_enc != 'UTF-8') {
										echo "<PRE>", mb_convert_encoding($s, 'UTF-8', $in_enc) , "</PRE>";
									} else {
										echo "<PRE>", $s , "</PRE>";
									}
								} else {
